...
public class IDEModule extends AbstractScriptModule {
	...
	public Optional<File> findFile(final Object resourceObj) {
		Object resolvedResourceObj = findResource(resourceObj).map(resource -> (Object) resource)
				.orElseGet(() -> ResourceTools.resolve(resourceObj));
	...
	@WrapToScript
	public Optional<Job> linkImportProject(final String projectName, final Object resourceObj)
	...
	@WrapToScript
	public Optional<Job> tryInitCopyImportProjectJob(final String projectName, final Object resourceObj,
			final boolean overwrite) throws CoreException, FileNotFoundException {
	...
	@WrapToScript
	public IFile touchFile(IProject project, String filePath, IProgressMonitor monitor) throws CoreException {
		IFile file = project.getFile(new Path(filePath));
		if (!file.exists()) {
			String defaultCharset = file.getParent().getDefaultCharset();
			file.create(IOUtils.toInputStream("", Charset.forName(defaultCharset)), false, monitor);
		}
		return file;
	}
